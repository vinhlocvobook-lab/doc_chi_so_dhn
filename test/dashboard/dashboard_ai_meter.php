...
define('DB_PORT', 3306);
define('DB_NAME', 'capnuoccangio');
define('DB_USER', 'capnuoc_cangio_user');
define('DB_PASS', '');
...
function buildWhere(array &$params): string
{
    ...
    if ($filterPrompt) {
        ...
        $params[':pv'] = $filterPrompt;
    }
    if ($filterImgType) {
        $where[] = 'image_type = :imgtype';
        $params[':imgtype'] = $filterImgType;
    }
...
function loaiSaiSoTag(string $loai): string
{
    return match ($loai) {
        ...
        default => '<span class="tag">' . htmlspecialchars($loai) . '</span>',
    };
}
    ...
    <header class="header">
        <div class="header-left">
            ...
            <div>
                ...
                <div class="header-sub">tn_meter_reading_log ·
                    <?= DB_NAME ?> ·
                    <?= date('d/m/Y H:i') ?>
                </div>
            </div>
    ...
    <div class="main">
            ...
            <div class="filter-bar">
                ...
                <select name="model" class="filter-select">
                    ...
                    <?php foreach ($modelList as $m): ?>
                        <option value="<?= $m['model_name'] ?>" <?= $filterModel === $m['model_name'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars($m['model_name']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>

                <select name="prompt" class="filter-select">
                    <option value="">Tất cả Prompt</option>
                    <?php foreach ($promptList as $pv): ?>
                        <option value="<?= $pv['prompt_version'] ?>" <?= $filterPrompt === $pv['prompt_version'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars($pv['prompt_version']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>

                <select name="imgtype" class="filter-select">
                    <option value="">Tất cả loại ảnh</option>
                    <option value="hinh_ro" <?= $filterImgType === 'hinh_ro' ? 'selected' : '' ?>>🟢 Hình rõ</option>
                    <option value="hinh_mo" <?= $filterImgType === 'hinh_mo' ? 'selected' : '' ?>>🟡 Hình mờ</option>
                    <option value="hinh_khong_day_du" <?= $filterImgType === 'hinh_khong_day_du' ? 'selected' : '' ?>>🟠
                        Không
                        đầy đủ</option>
                    <option value="hinh_khong_doc_duoc" <?= $filterImgType === 'hinh_khong_doc_duoc' ? 'selected' : '' ?>>
                        🔴
                        Không đọc được</option>
                </select>

                <div class="filter-divider"></div>
                <input type="date" name="from" class="filter-input" value="<?= htmlspecialchars($filterDateFrom) ?>">
                <span style="color:var(--muted);font-size:12px">→</span>
                <input type="date" name="to" class="filter-input" value="<?= htmlspecialchars($filterDateTo) ?>">
                <div class="filter-divider"></div>
        ...
        <div class="kpi-grid">
            <div class="kpi-card c-accent">
                ...
                <div class="kpi-value">
                    <?= number_format($kpi['tong_anh']) ?>
                </div>
                <div class="kpi-meta">
                    <?= $kpi['so_model'] ?> model ·
                    <?= $kpi['so_prompt'] ?> prompt version
                </div>
            </div>
            <div class="kpi-card c-green">
                <div class="kpi-label">Tỷ lệ chính xác (exact)</div>
                <div class="kpi-value">
                    <?= $kpi['ty_le_chinh_xac'] ?><span style="font-size:16px">%</span>
                </div>
                <div class="kpi-meta">
                    <?= number_format($kpi['exact_match']) ?> /
                    <?= number_format($tong) ?> ảnh đúng
                </div>
            </div>
            <div class="kpi-card c-yellow">
                ...
                <div class="kpi-meta">TB Score Thực tế:
                    <?= $kpi['avg_score_tt'] ?>
                </div>
            </div>
            <div class="kpi-card c-orange">
                ...
                <div class="kpi-value">
                    <?= number_format($kpi['can_review']) ?>
                </div>
                <div class="kpi-meta">
                    <?= pct($kpi['can_review'], $tong) ?>% tổng ảnh
                </div>
            </div>
            <div class="kpi-card c-red">
                <div class="kpi-label">Lỗi nghiêm trọng</div>
                <div class="kpi-value">
                    <?= number_format($kpi['loi_nghiem_trong']) ?>
                </div>
        ...
        <div class="row row-2">
            ...
            <div class="card">
                ...
                <div class="gauge-row">
                    <div class="gauge-wrap">
                        <div class="gauge-ring">
                            <svg viewBox="0 0 110 110">
                                ...
                                <circle class="fill" id="gpoc" cx="55" cy="55" r="46" stroke="var(--green)"
                                    stroke-dasharray="289" stroke-dashoffset="289" />
                            </svg>
                            <div class="gauge-center">
                                <div class="gauge-score" style="color:var(--green)">
                                    <?= $scorePocDist['avg_poc'] ?? 0 ?>
                                </div>
                    ...
                    <div style="flex:1">
                        <div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-bottom:10px">PHÂN
                            BỔ ĐIỂM TB</div>
                        <div class="sub-bar-list" style="margin-bottom:16px">
                            <div>
                                <div class="bar-item-header">
                                    ...
                                    <span style="color:var(--green);font-family:var(--mono)">
                                        <?= $scorePocDist['avg_so_sat'] ?> / 60
                                    </span>
                                </div>
                                <div class="bar-track">
                                    <div class="bar-fill"
                                        style="width:<?= pct($scorePocDist['avg_so_sat'], 60) ?>%;background:var(--green)">
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div class="bar-item-header">
                                    ...
                                    <span style="color:var(--accent);font-family:var(--mono)">
                                        <?= $scorePocDist['avg_ky_tu'] ?> / 40
                                    </span>
                                </div>
                                <div class="bar-track">
                                    <div class="bar-fill"
                                        style="width:<?= pct($scorePocDist['avg_ky_tu'], 40) ?>%;background:var(--accent)">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-bottom:8px">PHÂN
                            LOẠI MỨC ĐỘ</div>
                        <div class="bar-list">
                            <?php
                            $pocRows = [
                                ...
                                <div>
                                    <div class="bar-item-header">
                                        ...
                                        <span style="font-family:var(--mono);color:<?= $color ?>">
                                            <?= $cnt ?> <span style="color:var(--muted)">
                                                <?= $p_ ?>%
                                            </span>
                                        </span>
                                    </div>
                ...
                <div class="gauge-row">
                    <div class="gauge-wrap">
                        <div class="gauge-ring">
                            <svg viewBox="0 0 110 110">
                                <circle class="track" cx="55" cy="55" r="46" />
                                <circle class="fill" id="gtt" cx="55" cy="55" r="46" stroke="var(--accent)"
                                    stroke-dasharray="289" stroke-dashoffset="289" />
                            </svg>
                            <div class="gauge-center">
                                <div class="gauge-score" style="color:var(--accent)">
                                    <?= $scoreTTDist['avg_tt'] ?? 0 ?>
                                </div>
                                <div class="gauge-max">/100</div>
                            </div>
                        </div>
                        ...
                        <div class="sub-bar-list" style="margin-bottom:14px">
                            ...
                            foreach ($ttBars as [$val, $lbl, $max, $color]):
                                ...
                                <div>
                                    ...
                                    <div class="bar-track">
                                        <div class="bar-fill"
                                            style="width:<?= pct($val, $max) ?>%;background:<?= $color ?>">
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-bottom:8px">QUYẾT
                            ĐỊNH TỰ ĐỘNG</div>
                        <div class="decision-grid">
                            <div class="decision-card d-auto">
                                <div class="decision-icon">✅</div>
                                <div class="decision-num">
                                    <?= $scoreTTDist['cnt_auto'] ?>
                                </div>
                                <div class="decision-pct">
                                    <?= pct($scoreTTDist['cnt_auto'], $tong) ?>%
                                </div>
                            ...
                            <div class="decision-card d-watch">
                                <div class="decision-icon">👁</div>
                                <div class="decision-num">
                                    <?= $scoreTTDist['cnt_watch'] ?>
                                </div>
                                <div class="decision-pct">
                                    <?= pct($scoreTTDist['cnt_watch'], $tong) ?>%
                                </div>
                            ...
                            <div class="decision-card d-review">
                                <div class="decision-icon">🔍</div>
                                <div class="decision-num">
                                    <?= $scoreTTDist['cnt_review'] ?>
                                </div>
                                <div class="decision-pct">
                                    <?= pct($scoreTTDist['cnt_review'], $tong) ?>%
                                </div>
                            ...
                            <div class="decision-card d-reject">
                                <div class="decision-icon">❌</div>
                                <div class="decision-num">
                                    <?= $scoreTTDist['cnt_reject'] ?>
                                </div>
                                <div class="decision-pct">
                                    <?= pct($scoreTTDist['cnt_reject'], $tong) ?>%
                                </div>
                        ...
                        <tbody>
                            <?php foreach ($modelCompare as $m):
                                ...
                                <tr>
                                    ...
                                    <td class="mono">
                                        <?= number_format($m['tong_anh']) ?>
                                    </td>
                                    <td>
                                        <div class="mini-bar-wrap">
                                            <div class="mini-bar">
                                                <div class="mini-bar-fill"
                                                    style="width:<?= $m['ty_le_exact'] ?>%;background:<?= $color ?>"></div>
                                            </div>
                                            <span class="mono" style="color:<?= $color ?>;width:36px">
                                                <?= $m['ty_le_exact'] ?>%
                                            </span>
                                        </div>
                                    </td>
                                    <td><span class="score-pill <?= scoreClass((int) $m['avg_poc']) ?>">
                                            <?= $m['avg_poc'] ?>
                                        </span></td>
                                    <td><span class="score-pill <?= scoreClass((int) $m['avg_tt']) ?>">
                                            <?= $m['avg_tt'] ?>
                                        </span></td>
                                    <td class="mono"
                                        style="color:<?= $m['avg_chi_phi'] < 50 ? 'var(--green)' : ($m['avg_chi_phi'] < 100 ? 'var(--yellow)' : 'var(--red)') ?>">
                                        ₫
                                        <?= number_format($m['avg_chi_phi']) ?>
                                    </td>
                                    <td class="mono">
                                        <?= number_format($m['avg_tg']) ?>
                                    </td>
            ...
            <div class="card">
                ...
                <div class="roi-grid">
                    <div class="roi-card">
                        ...
                        <div class="roi-value" style="font-size:16px">₫
                            <?= number_format($roi['tong_chi_phi']) ?>
                        </div>
                        <div class="roi-note">
                            <?= number_format($tong) ?> ảnh
                        </div>
                    </div>
                    <div class="roi-card">
                        <div class="roi-label">Chi phí / ảnh TB</div>
                        <div class="roi-value" style="font-size:16px">₫
                            <?= number_format($roi['avg_chi_phi']) ?>
                        </div>
                    </div>
                    <div class="roi-card">
                        <div class="roi-label">Tự động 100%</div>
                        <div class="roi-value" style="color:var(--green);font-size:16px">
                            <?= $roi['pct_tu_dong'] ?>%
                        </div>
                    ...
                    <div class="roi-card">
                        ...
                        <div class="roi-value" style="color:var(--yellow);font-size:16px">
                            <?= $roi['pct_billing'] ?>%
                        </div>
                ...
                <div class="info-box green" style="margin-top:12px">
                    ...
                    if ($bestModel):
                        ...
                        echo ", chi phí <strong>₫" . number_format($bestModel['avg_chi_phi']) . "/ảnh</strong>.";
                    endif;
                    ?>
                    Ước tính tiết kiệm <strong style="color:var(--green)">
                        <?= $roi['pct_tu_dong'] ?>%
                    </strong> công nhân viên.
                </div>
            </div>
        </div>
        ...
        <div class="row row-2">
                ...
                <div class="error-grid">
                    ...
                    foreach ($errorDist as $e):
                        ...
                        <div class="error-card">
                            <div class="error-count" style="color:<?= $c ?>">
                                <?= number_format($e['so_luong']) ?>
                            </div>
                            <div class="error-name">
                                <?= htmlspecialchars($loai) ?>
                            </div>
                            <div class="error-pct" style="color:<?= $c ?>">
                                <?= $e['pct'] ?>%
                            </div>
                        </div>
                    <?php endforeach; ?>
            ...
            <div class="card">
                ...
                <div class="imgq-grid">
                    ...
                    foreach ($imgQuality as $iq):
                        ...
                        <div class="imgq-card">
                            ...
                            <div class="imgq-count">
                                <?= $iq['so_luong'] ?>
                            </div>
                            <div class="imgq-name">
                                <?= htmlspecialchars($iq['loai_anh']) ?>
                            </div>
                            <div class="imgq-acc" style="color:<?= $color ?>">
                                <?= $iq['ty_le_chinh_xac'] ?>%
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                <div style="font-size:11px;color:var(--muted);font-family:var(--mono);margin-bottom:8px">ĐỘ CHÍNH XÁC
                    THEO LOẠI ẢNH</div>
                <div class="bar-list">
                    <?php foreach ($imgQuality as $iq):
                        ...
                        <div>
                            ...
                            <div class="bar-track">
                                <div class="bar-fill" style="width:<?= $iq['ty_le_chinh_xac'] ?>%;background:<?= $color ?>">
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                ...
                <div class="info-box yellow">
                    ...
                    Đào tạo nhân viên chụp ảnh đúng góc &amp; đủ sáng có thể nâng độ chính xác thêm
                    <strong>10–15%</strong>.
                </div>
            </div>
        </div>
                    ...
                    <tbody>
                        <?php foreach ($recentLogs as $log): ?>
                            <tr>
                                ...
                                <td><span class="model-badge" style="font-size:9px">
                                        <?= htmlspecialchars(str_replace('gemini-2.5-', 'g2.5-', $log['model_name'])) ?>
                                    </span></td>
                                <td class="mono" style="color:var(--muted)">
                                    <?= $log['prompt_version'] ?>
                                </td>
                                ...
                                <td>
                                    <?= loaiSaiSoTag($log['loai_sai_so'] ?? '') ?>
                                </td>
                                <td><span class="score-pill <?= scoreClass((int) $log['score_poc']) ?>">
                                        <?= $log['score_poc'] ?>
                                    </span></td>
                                <td><span class="score-pill <?= scoreClass((int) $log['score_thuc_te']) ?>">
                                        <?= $log['score_thuc_te'] ?>
                                    </span></td>
                                <td style="white-space:nowrap">
                                    <?= mucDoTTLabel($log['muc_do_thuc_te'] ?? '') ?>
                                </td>
                                <td>
                                    <?= imgTypeLabel($log['image_type'] ?? '') ?>
                                </td>
                                <td class="mono">
                                    <?= number_format($log['thoi_gian_xu_ly']) ?>
                                </td>
                                <td class="mono"
                                    style="color:<?= $log['chi_phi_vnd'] < 50 ? 'var(--green)' : 'var(--yellow)' ?>">
                                    ₫
                                    <?= number_format($log['chi_phi_vnd'], 0) ?>
                                </td>
                                <td>
                                    <?= apiStatusTag($log['trang_thai_api'] ?? '') ?>
                                </td>
